...
impl Downloader {
    ...
    fn at_base_url(cache: ToolCache, platform: Platform, base_url: String) -> Downloader {
        Downloader {
            cache,
            platform,
            base_url,
...
/// return the path to its binary. See [`Downloader::ensure_installed`].
pub fn ensure_installed(
    spec: &ToolSpec,
    version: &str,
    ctx: &InstallContext,
...
fn fetch_message(label: &str, name: &str, version: &str, verbose: bool) -> String {
    if verbose {
...
mod tests {
    ...
    impl FixtureServer {
        ...
        fn serve_responses(routes: HashMap<String, (u16, Vec<u8>)>) -> FixtureServer {
                ...
                std::thread::spawn(move || {
                    for request in server.incoming_requests() {
                        ...
                        let response = match routes.get(&url) {
                            Some((status, bytes)) => tiny_http::Response::from_data(bytes.clone())
