//! Where downloaded tools live on disk:
...
/// The on-disk download cache, rooted at `<data_dir>/tools`.
    ...
    pub fn from_env() -> anyhow::Result<ToolCache> {
        match data_dir(std::env::var_os("HPDS_DATA_DIR")) {
            ...
            None => Err(anyhow::anyhow!(
                "could not determine a data directory for hpds downloads"
            ))
            .hint(
...
mod tests {
    ...
    fn layout_is_tools_name_version_under_the_data_dir() {
        ...
        assert_eq!(
            cache.tool_dir("gh", "2.96.0"),
            data.join("tools").join("gh").join("2.96.0")
        );
    }
    ...
    fn binary_path_uses_the_platform_binary_name() {
        ...
        assert_eq!(
            cache.binary_path("gh", "2.96.0", unix),
            cache.tool_dir("gh", "2.96.0").join("gh")
        );
        assert_eq!(
            cache.binary_path("gh", "2.96.0", windows),
            cache.tool_dir("gh", "2.96.0").join("gh.exe")
        );
    }
    ...
    fn manifest_lives_next_to_the_binary() {
        let cache = ToolCache::at(Path::new("data"));
        assert_eq!(
            cache.manifest_path("uv", "0.9.5"),
            cache.tool_dir("uv", "0.9.5").join("manifest.json")
        );
    }
